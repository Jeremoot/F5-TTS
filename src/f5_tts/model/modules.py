...
class DurationPredictor(nn.Module):
    ...
    def forward(
        self,
        inp: torch.Tensor,  # shape: (B, N, D) or (B, N, W)
        ...
        *,
        ...
        mask_repeated = repeat(mask, "b n -> b n d", d=self.num_channels).to(inp.device)
        inp = torch.where(mask_repeated, inp, torch.zeros_like(inp))
