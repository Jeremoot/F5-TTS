...
The checkpoints support English.

For tutorial and updates check here (https://github.com/Jeremoot/F5-TTS/)
"""
    )
...
```""")
            ...
            nfe_step = gr.Number(label="NFE Step", value=64)
            ch_use_ema = gr.Checkbox(label="Use EMA", value=True)
            speed_slider = gr.Slider(label="Speed", minimum=0.1, maximum=3.0, step=0.1, value=1.0)
            with gr.Row():
            ...
            # Reference samples map
            reference_map = {
                "2s.wav": "or suicides would probably be more frequent",
                "3s.wav": "but later admitted that he did but that quote",
                "4s.wav": "he condemned their ultra-humanitarianism as misplaced.",
                "5s.wav": "it had been dressed up in decent clothes and made to stand upright in the cart.",
                "6s.wav": "while the police work of the dallas authorities in the early identification and apprehension of oswald",
                "7s.wav": "it is only from their ruins that we may hope to obtain accurate information of the strongest fortifications in the ancient world.",
                "8s.wav": "nodded pleasantly to the governor who stood just opposite and then walked briskly towards the execution shed smiling as he went along.",
                "9s.wav": "from 1832 to 1844 not a single person had been executed in the metropolis except for this the gravest crime.",
                "10s.wav": "based on the foregoing considerations is convenient for distinguishing between animals and plants in all cases except the doubtful unicellular forms.",
            }
